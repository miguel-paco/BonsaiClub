...
def calculate_velocities(value):
    ...
    print(diffCoord_x)
    diffCoord_x = diffCoord_x * fr

    #print(diffCoord_x)
    diffCoord_y = (present_y - past_y)
    diffCoord_y = diffCoord_y * fr
    ...
    #if diffori > 2pi set the value to previous
    print(diffOri)
    VelAngular_Degs = ((np.degrees(diffOri)*fr))#-1.0271566491397963) / 202.38676608044608
    Walldist = ((55 - np.sqrt(np.multiply(X,X) + np.multiply(Y,Y))))#-34.71692942696893) / 5.420970792185561
    loaded_clf = TabNetClassifier()
    loaded_clf.load_model('C:/Users/Admin/Desktop/tabnet_100epochs_just_vel_othersave.h5.zip')
    model = loaded_clf
    print(model)
    ...
    y_pred = model.predict_proba(dataframe_with_data)
    print(y_pred)
    output = pd.DataFrame(y_pred[0])
